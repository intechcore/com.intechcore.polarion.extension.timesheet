...
```bash
mvn clean package            # do NOT pass -s .mvn/settings.xml (default settings resolve repos)
```

- The React UI build is **inherited from the generic parent** (its `vite-ui` profile, activated
...
- **Don't hardcode `Require-Bundle`** in `META-INF/MANIFEST.MF`. The list is supplied by the
  `generic` 15.x parent (so it tracks the correct 2606 bundle names, e.g.
...
- **Commits**: a pre-commit hook requires `user.email` to match
  `firstname.lastname@intechcore.com`. Migration/breaking commits use `feat!: ...` so
  release-please bumps the major version.
